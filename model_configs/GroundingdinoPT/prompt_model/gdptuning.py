...
class GroundingDINOPTuning(GroundingDINO):
    ...
    def predict(self, batch_inputs, batch_data_samples, rescale: bool = True):
        ...
        for data_samples in batch_data_samples:
            ...
            text = list(text)
            #print('text before reading prompt_pth: ',text) # only for debugging...
            ...
            if hasattr(data_samples, 'prompt_pth'):
                ...
                text_prompts.append(pseudo_words)
                #print('label_list after reading prompt_pth: ',label_list) # only for debugging...
                #print('text_prompts after reading prompt_pth: ',text_prompts) # only for debugging...
        ...
        # extract text feats
        #print('text_prompts before using language model: ',text_prompts) # only for debugging...
        text_dict = self.language_model(list(text_prompts))
        #print('text dict after using language model: ',text_dict) # only for debugging...
        ...
        if hasattr(data_samples, 'prompt_pth'):
            ...
            for i in range(len(visual_query)):
                cur = visual_query[i].repeat(text_dict['embedded'].shape[0], 1,
                                             1)
                #print(cur.shape) # only for debugging...
                text_dict['embedded'][:,
                                      positive_map[i +
                                                   1][0]:positive_map[i +
                                                                      1][-1] +
                                      1, :] = cur
                #print('text_dict[\'embedded\'] shape: ', text_dict['embedded'].shape) # only for debugging...
        else:
            positive_map = token_positive_maps[0]
            for i in range(self.class_num):
                cur = self.learning_prompts[i].weight.repeat(
        ...
        #print(text_dict['embedded'].shape)  # only for debugging...
